...
class BaseFiltersDb(MongoDb):
    ...
    async def count_documentsV2(self, channel_id: dict) -> int:  # Add chat_id as a type-hinted parameter
        result = await self.col.count_documentsV2({'chat_id': channel_id})
        return result
...
class FiltersDb2(BaseFiltersDb):
    def __init__(self):
        super().__init__(Config.COLLECTION_NAME2)

class FiltersDb3(BaseFiltersDb):
    def __init__(self):
