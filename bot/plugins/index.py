...
@Bot.on_callback_query(filters.regex(r"^ch4index"))  # type: ignore
async def index_filesch4(bot: Bot, query: types.CallbackQuery):
    if query.data.startswith("index_cancel 3"):  # type: ignore
        Cache.CANCEL = True  # type: ignore
        return await query.answer("Cancelling Indexing 3")
    _, sts, chat, lst_msg_id, from_user = query.data.split("#")  # type: ignore
    ...
    try:
        ...
        chat = chat
    await index_files_to_ch4db(int(lst_msg_id), chat, msg, bot)  # type: ignore
    ...
    if message.from_user.id in Config.ADMINS:
        buttons = [
            ...
            [
                InlineKeyboardButton(
                    "Yes3",
                    callback_data=f"ch4index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}",
                )
            ],
            [
...
async def index_files_to_ch4db(lst_msg_id: int, chat: int, msg: types.Message, bot: Bot):
    total_files = 0
    duplicate = 0
    errors = 0
    deleted = 0
    ...
    async with lock:
        try:
            ...
            async for message in bot.iter_messages(chat, lst_msg_id, Cache.CURRENT):  # type: ignore
                if Cache.CANCEL:
                    inserted, errored = await d_filter.insert_pending()
                    if inserted:
                        total_files += inserted
                    if errored:
                        duplicate += errored
                    await msg.edit(
                        f"4Successfully Cancelled!!\n\nSaved <code>{total_files} / {current}</code> files to dataBase!\nDuplicate Files Skipped: <code>{duplicate}</code>\nDeleted Messages Skipped: <code>{deleted}</code>\nNon-Media messages skipped: <code>{no_media + unsupported}</code>(Unsupported Media - `{unsupported}` )\nErrors Occurred: <code>{errors}</code>"
                    )
                    break
                current += 1
                if current % 200 == 0:
                    ...
                    reply = InlineKeyboardMarkup(can)
                    await msg.edit_text(
                        text=f"4Total messages fetched: <code>{current}</code>\nTotal messages saved: <code>{total_files}</code>\nDuplicate Files Skipped: <code>{duplicate}</code>\nDeleted Messages Skipped: <code>{deleted}</code>\nNon-Media messages skipped: <code>{no_media + unsupported}</code>(Unsupported Media - `{unsupported}` )\nErrors Occurred: <code>{errors}</code>",
                        reply_markup=reply,
                    )
                if message.empty:
                    deleted += 1
                ...
                media.channel_name = message.chat.username or message.chat.title
                media.message_id = message.id
                inserted, errored = await d_filter.insert_many(media)
                if inserted:
                    total_files += inserted
                if errored:
                    duplicate += errored
            ...
            if errored:
                duplicate += errored
            await msg.edit(
                f"4Successfully saved <code>{total_files} / {current}</code> to dataBase!\nDuplicate Files Skipped: <code>{duplicate}</code>\nDeleted Messages Skipped: <code>{deleted}</code>\nNon-Media messages skipped: <code>{no_media + unsupported}</code>(Unsupported Media - `{unsupported}` )\nErrors Occurred: <code>{errors}</code>"
            )
