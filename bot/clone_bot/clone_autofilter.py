...
async def clone_give_filter(bot: Bot, message: types.Message):
    ...
    if 2 < len(message.text) < 100:
        ...
        files, offset, total_results = await a_filter.get_search_results(
            search.lower(), offset=0, filter=True, photo=settings['PHOTO_FILTER'], video=settings['VD_FILTER']
        )
        if not files:
            return
    else:
        ...
        if offset != "":
            ...
            btn.append(
                [
                    ...
                    types.InlineKeyboardButton(
                        text="NEXT ⏩", callback_data=f"clonenext_{req}_{key}_{offset}"
                    ),
                ]
            )
        else:
...
@Bot.on_callback_query(filters.regex(r"^clonenext"))  # type: ignore
async def clone_next_page(bot: Bot, query: types.CallbackQuery):
    ...
    if n_offset == 0:
        btn.append(
            [
                types.InlineKeyboardButton(
                    "⏪ BACK", callback_data=f"clonenext_{req}_{key}_{off_set}"
                ),
                types.InlineKeyboardButton(
        ...
        btn.append(
            [
                ...
                types.InlineKeyboardButton(
                    "NEXT ⏩", callback_data=f"clonenext_{req}_{key}_{n_offset}"
                ),
            ]
        )
    else:
...
async def clone_handle_file(bot: Bot, query: types.CallbackQuery):
